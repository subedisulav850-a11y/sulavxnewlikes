...
def add_account():
    ...
    Usage: /add_account?uid=123456&pass=xyz&region=IND&key=Sulav
    """
...
def reset_cache():
    ...
    if key != "Sulav":
        return jsonify({"error": "Invalid key"}), 403
    liked_cache.clear()
    return jsonify({"message": "Cache cleared", "credit": "@SulavOFFICIAL"})

if __name__ == '__main__':
    print("🚀 Smart Like API with Account Manager")
    print("📍 Endpoints:")
    print("   GET  /like?uid=UID&server_name=REGION&key=Sulav")
    print("   GET  /add_account?uid=...&pass=...&region=...&key=Sulav")
    print("   GET  /reset-cache?key=Sulav")
